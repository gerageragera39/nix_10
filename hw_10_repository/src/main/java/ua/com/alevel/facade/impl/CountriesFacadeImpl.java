...
public class CountriesFacadeImpl implements CountriesFacade {
    ...
    public CountriesResponseDto findById(Long id) {
        return new CountriesResponseDto(countiesService.findById(id).get());
    }

    @Override
    public PageData<CountriesResponseDto> findAll(WebRequest request, boolean visible) {
        ...
        DataTableResponse<Countries> dataTableResponse = countiesService.findAll(dataTableRequest, visible);
